...
## Dependency and mutually exclusiveness

I two events A and B do not affect each other, they are called independent events. 
...
A = An ace appears when a card drawn from a deck of 52 cards
...
If event A did not happend, P(B) would be $\frac 1 {52}$, so it turns out B depends on A. 

Now, two events are called mutually exclusive when occurrence of one prevents other from happening. 

If a die is thrown once one of 1-6 will face up. If 1 appears, 2, or any other number from the rest cannot appear. Thus, Getting these numbers are mutually exclusive or disjoint. Mutually exclusive sets do not have any common elements. 

Now, merging the concepts of dependency and mutually exclusiveness is tricky. 
...
Now, let us prove it theoretically. 
