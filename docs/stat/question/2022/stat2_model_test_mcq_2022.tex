...
\choice{$P(\bar A \cap B) = P(B) - P(A \cup B)$}{$P(\bar A \cap B) = P(B) - P(A \cap B)$}{$P(\bar A \cup B) = P(B) - P(A \cup B)$}{$P(\bar A \cap B) = P(A) - P(A \cap B)$}{b}
...
\question Expectation for a discrete variable x is defined as: 
\choice {$\displaystyle \int_{-\infty}^{\infty} x_if(x_i) dx$}{$\displaystyle \sum_{i=-\infty}^{0}x_ip(x_i)$}{$\displaystyle \sum_{i=-\infty}^{\infty}x_ip(x_i)$}{$\displaystyle \sum_{i=-\infty}^{\infty}x_ip(x_i^2)$}{a}
...
\question If $Y=aX+b, E(Y)=?$
\choice {$aE(X)+b$}{$a^2E(X)$}{$E(X)$}{$a+bE(X)$}{a}

\question Expectation is equal to-
\choice {Variance}{Square of variance}{Arithmetic mean}{Standard deviation}{a}

\question If $E(X)=2$ and $E(X^2)=8$, what is the standard deviation?
\choice {0}{2}{4}{8}{a}

\question $f(x)=5x^4; 0\le x \le 1, E(X)=?$
\choice {0.0204}{0.833}{0.9204}{1}{a}

\question The mean of the binomial distribution is
\choice{$np$}{$nq$}{$npq$}{$\sqrt{npq}$}{a}

\question What is true of binomial distribution?
\choice{$np=0$}{$np < 0$}{$np > 0$}{$np \ne 0$}{a}

\question If a coin is tossed once, it is called 
...
\choice{i \& ii}{i \& iii}{i}{i, ii, \& iii}{a}

\question If the mean of a Poisson distribution is 4, what is its variance?
\choice{2}{3}{4}{16}{a}

\question If a Poisson distribution is defined as $P(x) = \frac{e^{-4} 4^x}{x!}$, what is the value of $P(X \le 1)$
\choice{0.09}{0.02}{0.07}{0.24}{a}
...
\question When is a Binomial distribution symmetric?
\choice{$p < q$}{$p > q$}{$p=q^2$}{$p=q$}{a}
